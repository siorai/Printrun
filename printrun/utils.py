...
def set_utf8_locale():
    ...
    # Load in locale from *nix 
    lang, encoding = locale.getlocale()
    # On windows getlocale will return None for lang.
    # This runs the Get-Culture cmdlet and formats it appropriately.
    if lang is None:
        ps_path = r'C:\WINDOWS\system32\WindowsPowerShell\v1.0\powershell.exe'
        # Checks for env PSPATH for users that have alternative PS exe to use
        if os.getenv('PSPATH'):
            ps_path = os.getenv('PSPATH')
        ps_cmd = r'(Get-Culture).Name'
        lang = str(subprocess.run([ps_path, ps_cmd], 
                              stdout=subprocess.PIPE).stdout)
        lang = lang[2] + lang[3] + '_' + lang[5] + lang[6]
    if encoding != 'UTF-8':
        locale.setlocale(locale.LC_ALL, lang + r'.' + r'UTF-8')
